...
function accessGrantedTemplate(r: TemplatePayload['record']) {
  const first = (r.name || 'there').trim().split(' ')[0]
...
function adminAlert(r: TemplatePayload['record']) {
  ...
  return { subject: `New signup #${r.position ?? '—'} — ${r.name || r.email}`, html }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const adminInbox = Deno.env.get('Waitlist') || SENDER_EMAIL
    ...
    if ('template' in body && body.template) {
      ...
      if (body.template === 'welcome') {
        const t = welcomeTemplate(r)
        sends.push({ to: [r.email], subject: t.subject, html: t.html })
        const a = adminAlert(r)
        sends.push({ to: [adminInbox], subject: a.subject, html: a.html })
      } else if (body.template === 'access_granted') {
        const t = accessGrantedTemplate(r)
        sends.push({ to: [r.email], subject: t.subject, html: t.html })
      } else {
